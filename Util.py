...
def agregarImagen(ventana, archivo, fila, columna, expandir=1, devolverTamaño=False):
    #cargar la imagen
    ...
    if devolverTamaño:
        return img.width(), img.height(), lbl
    else:
        return lbl
...
def mostrarTabla(ventana, encabezados, datos, fila, columna, ancho, alto, expandirFilas=1, expandirColumnas=1):
    tabla = VistaTabla(ventana, encabezados, datos, fila, columna, ancho, alto, expandirFilas, expandirColumnas)
...
class VistaTabla(object):
    ...
    #Metodo constructor
    def __init__(varClase, ventana, encabezados, datos, fila, columna, ancho, alto, expandirFilas=1, expandirColumnas=1):
        varClase.arbol = None #objeto para el despliegue de la tabla
        varClase.crear(ventana, encabezados, fila, columna, ancho, alto, expandirFilas, expandirColumnas)
        varClase.configurar(encabezados, datos)

    #Metodo que crea los objetos para despliegue
    def crear(varClase, ventana, encabezados, fila, columna, ancho, alto, expandirFilas, expandirColumnas):
        #crear el contenedor
        frm = Frame(ventana)
        frm.grid(row=fila, column=columna)
        if expandirFilas>1:
            frm.grid(rowspan=expandirFilas)
        if expandirColumnas>1:
            frm.grid(columnspan=expandirColumnas)
        ...
        frm.grid_columnconfigure(0, weight=1, minsize=ancho)
        frm.grid_rowconfigure(0, weight=1, minsize=alto)
    ...
    #Metodo que retorna el objeto Treeview que despliega los datos
